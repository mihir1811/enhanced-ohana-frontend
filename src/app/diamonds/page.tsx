    ...
    return (
      <button
        ...
        className={`p-4 rounded-lg border-2 transition-all ${
          isSelected ? selectedConfig.colors : 'border-gray-200 hover:border-gray-300'
        ...
        return (
          <button
            ...
            className={`p-3 rounded-lg border text-sm font-medium transition-all ${
              isSelected ? colorConfig[colorVariant] : 'border-gray-200 hover:border-gray-300'
            }`}
            style={{
  ...
  return (
      <div className="min-h-screen" style={{ backgroundColor: 'var(--background)' }}>
        ...
        <div className={`max-w-[${SECTION_WIDTH}px] mx-auto px-4 py-12`}>
          ...
          <div className='mb-6'>
            <MultiSelectFilter
              options={DIAMOND_CONSTANTS.PROCESS_OPTIONS}
              selected={searchForm.process}
              onChange={handleMultiSelect}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                {DIAMOND_CONSTANTS.SHAPES.slice(0, 12).map((shape) => (
                  <button
                    ...
                    style={{
                      backgroundColor: searchForm.shape.includes(shape) ? 'var(--blue)/10' : 'var(--card)',
                      borderColor: searchForm.shape.includes(shape) ? 'var(--blue)' : 'var(--border)',
                      color: searchForm.shape.includes(shape) ? 'var(--blue)' : 'var(--foreground)'
                    }}
                  >
                    {shape}
                  </button>
            ...
            <div className="mt-8 border-t pt-6" style={{ borderColor: 'var(--border)' }}>
              ...
              {showAdvancedFilters && (
                <div className="mt-8 space-y-6 p-6 rounded-xl border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)/5' }}>
                  ...
                  <MultiSelectFilter
                    options={DIAMOND_CONSTANTS.CULET_OPTIONS}
                    selected={searchForm.culet}
                    onChange={handleMultiSelect}
                    label="Culet"
                    gridCols="grid-cols-2 md:grid-cols-3"
                  />
          ...
          <div className="mt-8 space-y-6 p-6 rounded-xl border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)/5' }}>
            <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--foreground)' }}>Extended Filters</h3>
